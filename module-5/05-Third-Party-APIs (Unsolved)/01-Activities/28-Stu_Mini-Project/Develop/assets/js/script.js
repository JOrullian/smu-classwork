...
function createProjectCard(project) {
  ...
  const cardHeader = $('<div>').addClass('card-header h-4').text(project.name);
  ...
  const cardDue = $('<p>').addClass('card-text').text(project.dueDate);
  ...
  const deleteButton = $('<btn>').addClass('btn btn-danger delete').text('Delete').attr('data-project-id', project.id)
  ...
  cardBody.append(cardType, cardDue, deleteButton)
...
function handleProjectFormSubmit(event) {
  ...
  const getData = JSON.parse(localStorage('projects'));
